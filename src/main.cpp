...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lulzcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lulzcoin Signed Message:\n";
...
{
    // Lulzcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // BIP16 didn't become active until  Wed, 30 Jul 2014 23:59:59 GMT
    int64 nBIP16SwitchTime = 1406764799;
...
{
    // Lulzcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
        ...
        // across the hard forks for Lulzcoin in particular
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Lulzcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LulzcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LulzcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LulzcoinMiner(CWallet *pwallet)
{
    printf("LulzcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lulzcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LulzcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LulzcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LulzcoinMiner, pwallet));
}
